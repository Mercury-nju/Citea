    ...
    if (localUser) {
      ...
      try {
        ...
        if (hasSupabaseService) {
          console.log('[Resend] Supabase 邮件服务可用，正在发送 Magic Link 验证邮件...')
          
          const emailResult = await sendVerificationEmail(email, newVerificationCode, localUser.name || email)
          
          if (emailResult.success) {
            console.log(`[Resend] ✅ Magic Link 邮件发送成功! MessageId: ${emailResult.messageId}`)
            emailSent = true
            message = '验证邮件已重新发送！请检查您的邮箱。'
          } else {
            console.error('[Resend] ❌ Magic Link 邮件发送失败:', emailResult.error)
            emailError = emailResult.error || '邮件发送失败'
            message = '邮件发送失败，请稍后重试。'
          }
        } else {
          console.log('[Resend] ⚠️ Supabase 邮件服务未配置')
          emailError = '邮件服务未配置'
